 ...
 * HTTP response headers}, but related to DMR responses only.
 * <p>
 * In standalone mode there's only a single {@code response-headers} node in the DMR response, whereas in domain mode
 * the response headers are scoped to a specific server group, host and server. The {@link #process(Header[])} method
 * will only receive the {@code response-headers} payload w/o the surrounding 'noise'.
 * <p>
 * Standalone Mode:
 * <pre>
 * {
